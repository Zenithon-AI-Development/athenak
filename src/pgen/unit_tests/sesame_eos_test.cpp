...
//========================================================================================
//! \file sesame_eos_test.cpp
//  \brief Unit test: the SESAME 3T EOS reader + the common 3T EOS representation
//  (eos/sesame_eos_reader.hpp + eos/eos_table_3t.hpp).  Reads a committed SESAME-style
//  fixture material (electron table 304 + ion table 305 + ionization table 601), then on
//  the device checks that the parsed grid, the per-species electron/ion specific energies
//  / pressures / mean ionization, the heat capacities the reader DERIVES from the energy
//  (c_v = de/dT), the (rho,T) interpolation, the e->T inversion the representation feeds,
//  and the table-edge clamping all reproduce the fixture's known synthetic laws (issue
//  [13c]/#11, ADR-0007).
//
//  The fixture (inputs/unit_tests/sesame_eos_test.ses) stores a synthetic 3T-native EOS
//  on a log-spaced (T, rho) grid (T [eV], rho [g/cc]; specific energies):
...
//  The e/p laws are power laws (affine in log10) so the log-log bilinear interpolation
//  reproduces them exactly on AND off grid; zbar is affine in (log10 rho, log10 T) so the
//  raw bilinear interpolation reproduces it exactly too.  Because e is LINEAR in T, the
//  reader's central/one-sided finite-difference c_v = de/dT recovers the generating
//  c_v(rho) exactly at every node -- so we can assert the derived heat capacities against
//  the closed-form laws to round-off, and the e->T inversion recovers the generating
//  temperature to the Newton tolerance.
//
//  The fixture path is passed on the command line as `problem/eos_file=<abspath>` by the
//  Python wrapper (tst/test_suite/unit_tests/test_unit_sesame_eos_cpu.py).
//
//  Built/run by
//    cmake -D PROBLEM=unit_tests/sesame_eos_test -B build_unit
//    (cd build_unit/src && make) && ./build_unit/src/athena \
//        -i inputs/unit_tests/sesame_eos_test.athinput \
//        problem/eos_file=$PWD/inputs/unit_tests/sesame_eos_test.ses
...
#include "eos/sesame_eos_reader.hpp"
...
//! \brief SESAME 3T EOS reader + common-representation unit test.

void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  unit_test::UnitTest test("sesame_eos_test");

  // --- read the SESAME fixture material from disk into the common representation ---
  std::string fname = pin->GetString("problem", "eos_file");
  eos_table_3t::EosTable3T table;
  eos_table_3t::ReadSesameEos(fname, table);
  ...
  // Heat capacities are DERIVED by the reader (c_v = de/dT); exact for the linear-in-T
  // fixture, so they must reproduce the closed-form c_v(rho).
  test.CheckNear(h_vals(ICV_E), Cvele(rho_og), rt, 0.0,
                 "CvEle == 1.5 rho^0.10 (derived de_e/dT, off-grid)");
  test.CheckNear(h_vals(ICV_I), Cvion(rho_og), rt, 0.0,
                 "CvIon == 0.9 rho^0.05 (derived de_i/dT, off-grid)");
