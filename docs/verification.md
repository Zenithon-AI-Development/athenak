...
| `test_verify_nernst_advection_cpu.py` | 1 | analytic | **RED-FIRST (#238, ADR-0017 Proposed — xfail until the Nernst operator lands.)** Passive B_z bump on a static uniform-p T_e ramp translates rigidly at the Braginskii Nernst velocity v_N=−(β_∧(x_e)/x_e)(τ_e/m_e)∇(k_B T_e); measured centroid displacement vs v_N·tlim | closed form (rigid advection); Braginskii 1965, Haines 1986, Nishiguchi 1984; arXiv:2504.10760 §2.1.2 | self |
...
| `test_verify_strang_coupling_cpu.py` / `_mpicpu.py` | 1 | analytic (point-implicit quartic + conservation) | Strang-split coupled timestep wired into the MHD driver (`mhd/strang_split`, #115/[B2], ADR-0009): the stiff parabolic block (grey FLD) runs as one RKL2 super-step over a per-field `CompositeParabolicOperator`, Strang-wrapped around the hyperbolic update as (½·STS · hydro · ½·STS), with the per-cell point-implicit matter-radiation coupling wrapped OUTSIDE the super-step (½ coupling each side). **uniform mode** (coupling-outside-the-super-step): uniform E_r on a uniform gas at rest makes the FLD diffusion the identity and hydro inert, yet the super-step still runs s=25 substages — after one step (E_r, e_gas) equals EXACTLY two dt/2 backward-Euler coupling steps (host #23 quartic oracle) to round-off, which it would NOT if the coupling were applied 2s times inside the substage loop; also asserts s>2 and conserves E_r+e_gas. **bump mode** (fully-coupled conservation): a smooth radiation bump on a uniform gas, insulated FLD + coupling + conservative hydro on 2 MeshBlocks (cpu) / 2 MPI ranks (mpicpu) — the volume-weighted total (E_r+E_gas) is invariant to round-off across the species split; the MPI case also confirms the per-field composite's global min-dt reduce (#114) keeps the spatially-varying-diffusivity cross-rank `SyncParabolicGhosts` from deadlocking. Embedded RED→GREEN: a half-step dt set to the full dt breaks the uniform oracle | closed form (backward-Euler emission/absorption quartic, #23); exact conservation; Meyer, Balsara & Aslam 2014 (RKL2); ADR-0009 (Strang ½·STS·hydro·½·STS, coupling outside) | `test_verify_composite_parabolic_cpu.py`, `test_unit_matter_radiation_coupling_cpu.py` |
...
| `test_verify_maglif_mrt_cpu.py` | 1 | literature | FLASH benchmark 1 — single-mode MRT: seeded-mode amplitude growth on the magnetically-driven liner interface under sustained acceleration. **Phase B (#116/[B3]):** now runs the FULL COUPLED radiation-conduction-MHD stack (`strang_split` + grey FLD + anisotropic conduction + matter-radiation coupling); GROWTH_MIN re-anchored 1.5→1.3 for the coupled regime (conduction stabilises the short mode); + species-split energy-budget check (`maglif_coupled_energy`). Baseline re-captured (coupled, not ideal-MHD) | Ellison et al. 2025 (arXiv:2504.10760) | self |
| `test_verify_maglif_mmrt_cpu.py` | 1 | literature | FLASH benchmark 2 — multi-mode MRT from surface roughness: broadband RMS growth + spectral participation (stays multi-mode). **Phase B (#116/[B3]):** full coupled stack + species-split energy check; GROWTH_MIN 1.5→1.3; baseline re-captured (coupled) | Ellison et al. 2025 (arXiv:2504.10760) | self |
| `test_verify_maglif_rm_cpu.py` | 1 | literature | FLASH benchmark 3 — converging single-mode Richtmyer–Meshkov: single-mode amplitude growth during compression. **Phase B (#116/[B3]):** full coupled stack + species-split energy check; GROWTH_MIN 1.5→1.3; baseline re-captured (coupled) | Ellison et al. 2025 (arXiv:2504.10760) | self |
| `test_verify_maglif_icf_cpu.py` | 1 | literature | FLASH benchmark 4 — ICF confinement-time signature: bulk stagnation via enclosed-mass tracking (R_if, ρ_fuel). **Phase B (#116/[B3]):** full coupled stack (own p0=0.1 coefficients) + species-split energy check; baseline re-captured (coupled) | Ellison et al. 2025 (arXiv:2504.10760) | self |
| `test_verify_maglif_rm_si_gpu.py` | 1 | literature (experiment) | **Phase C (#119/[C2]) QUANTITATIVE ANCHOR.** SI-dimensional converging-RM replication of the Knapp 2020 "Decel" platform (`inputs/maglif_rm_si.athinput`: Be liner piston → liquid-D₂ → on-axis Be rod, λ=300 µm / h₀=15 µm seed) on the GPU at uniform fine resolution (3-D cylindrical AMR is unsupported, `coordinates.cpp:62`/#38). Validates DIMENSIONLESS, convergence-governed observables against the committed experimental ground-truth oracle (`ground_truth/b3_convergent_rm_knapp_2020.json`) within max(exp, digitization) band: the RM amplitude **growth factor** (`rm_growth_factor` 4.8 ± 1.92) measured vs the post-shock minimum (Fig. 8), and the **shock/interface velocity ratio** (`rm_shock_interface_velocity_ratio` 1.369 ± 0.16, PDV). Coupled stack runs as a gentle perturbation; GPU-only (excluded from CPU CI). | Knapp PoP 27,092707 (2020) [10.1063/5.0013194] | `test_verify_ground_truth_oracle_cpu.py` |
| `test_verify_maglif_smoke_cpu.py` | 2 | qualitative-signature | **Phase B (#117/[B4]) per-PR integration-rot guard.** Low-res (32×4×24), MULTI-BLOCK (2 blocks in z), short (tlim=0.2) single-mode-MRT run on the FULL COUPLED stack (`strang_split` + grey FLD + anisotropic conduction + matter-radiation coupling). NOT baseline-pinned: asserts only that the coupled stack runs (finite final state), the species-split `erad` budget is sane (`maglif_coupled_energy`), the liner converges, and the seeded MRT mode grows. The integration-rot discriminator is the presence of the `erad` column (absent ⇒ coupled stack off ⇒ fails). Runs in ~9 s on every PR (`run_test_suite.py --cpu`) | qualitative MRT signature (Layer-1 anchor: `test_verify_maglif_mrt_cpu.py`) | self |
