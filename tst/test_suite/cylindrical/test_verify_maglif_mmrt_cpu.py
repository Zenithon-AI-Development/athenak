...
moves inward, (3) the broadband roughness grows on the driven surface, (4) the driven
surface roughens MORE than the RT-stabilised fuel-side surface (the MagLIF asymmetry),
(5) the seeded spectrum stays multi-mode (no single-mode collapse), and (6) the synthetic-
radiograph modulation grows, then plots the trajectories + a synthetic radiograph and
saves/diffs a golden regression baseline.  Auto-collected by run_test_suite.py (the
...
# >= 1.3x broadband-roughness growth on the driven surface.  Re-anchored from the ideal
# 1.5x for the coupled regime (#116/[B3]): anisotropic conduction stabilises the shorter
# axial modes, so coupled multi-mode growth is below the ideal; 1.3x keeps a comparable
# margin (the seeded roughness still grows substantially under the coupled stack).
GROWTH_MIN = 1.3
...
def test_verify_maglif_mmrt():
    ...
    try:
        ...
        # the multi-mode MRT growth signature.
        assert rms_outer[-1] > GROWTH_MIN * rms_outer[0], (
            f"driven-surface roughness did not grow: {rms_outer[0]:.4f} -> "
            f"{rms_outer[-1]:.4f} (need > {GROWTH_MIN}x)"
        )
