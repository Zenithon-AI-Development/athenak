"""
Auto-collected wrapper for the cn4 EOS dimensional-anchor pgen-based unit test
(#209/#210).

Builds src/pgen/unit_tests/ionmix_cn4_units_test.cpp
(-D PROBLEM=unit_tests/ionmix_cn4_units_test) into the isolated tst/build_unit directory,
...
def test_run():
    ...
    passed = testutils.run_unit_test(
        "ionmix_cn4_units_test",
        args=[f"problem/eos_table={table}"],
    )
    assert passed, (
        "ionmix_cn4_units_test reported a failing check (nonzero exit)"
    )
